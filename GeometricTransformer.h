...
public:
	...
	Tham số
		...
		- pDst: con trỏ ảnh mới sau khi nội suy màu
	...
	*/
	virtual void Interpolate(
		float tx, float ty, uchar* pSrc, uchar* pDstRow, int srcWidthStep, int nChannels) = 0;
	PixelInterpolate();
	~PixelInterpolate();
};
...
{
public:
	void Interpolate(float tx, float ty, uchar* pSrc, uchar* pDst, int srcWidthStep, int nChannels);
	BilinearInterpolate();
	~BilinearInterpolate();
};
...
public:
	...
	void Inverse();// trả về ma trận nghịch đảo của _matrixTransform
	void Reflection(int opt);
	AffineTransform();
	~AffineTransform();
};
